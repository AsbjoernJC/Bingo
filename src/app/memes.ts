export type Meme = {
  number: number;
  meme: string;
};

export class Memes {
  memes: Meme[] = [
    {
      number: 1,
      meme: 'https://media.tenor.com/k7k0Im4PDUUAAAAd/we-are-number1.gif',
    },
    {
      number: 3,
      meme: 'https://y.yarn.co/c8ed7c84-3f89-4e02-9d85-b8727525e2b5_text.gif',
    },
    {
      number: 6,
      meme: 'https://media.tenor.com/jUMex_rdqPwAAAAd/among-us-twerk.gif',
    },
    {
      number: 7,
      meme: '../assets/7.gif',
    },
    {
      number: 8,
      meme: 'https://media.tenor.com/skwciv9sAiIAAAAd/avril-lavigne-sk8er-boi.gif',
    },
    {
      number: 9,
      meme: 'https://64.media.tumblr.com/tumblr_m6nrxzLckM1ru8yv8o8_250.gifv',
    },
    {
      number: 10,
      meme: 'https://media.tenor.com/fo0rUUBalvoAAAAC/bowling-ball-crash.gif',
    },
    {
      number: 11,
      meme: 'https://media.tenor.com/6lota1QE_fQAAAAC/screaming-eleven.gif',
    },
    {
      number: 13,
      meme: 'https://media.tenor.com/qQMIkScfFswAAAAd/its-friday-the13th-dancing.gif',
    },
    {
      number: 15,
      meme: 'https://media.tenor.com/luPargXFICQAAAAd/walking-kitchen.gif',
    },
    {
      number: 17,
      meme: 'https://media.tenor.com/zOgBDchsoJYAAAAC/abba-dancing-queen.gif',
    },
    {
      number: 21,
      meme: 'https://media.tenor.com/rOaZl9xBDfwAAAAd/vine-21.gif',
    },
    {
      number: 22,
      meme: '../assets/22.gif',
    },
    {
      number: 23,
      meme: 'https://media.tenor.com/dBFMInJknQkAAAAC/michael-jordan-jordan.gif',
    },
    {
      number: 24,
      meme: 'https://media.tenor.com/7o97DimLGe0AAAAC/24k-magic-bruno-mars.gif',
    },
    {
      number: 42,
      meme: 'https://media.tenor.com/T-weCEHq2BgAAAAC/life-the-universe-and-everything-hitchhikers-guide-to-the-galaxy.gif',
    },
    {
      number: 47,
      meme: 'https://media.tenor.com/s5KWdhN1PfgAAAAd/gold-arabesque.gif',
    },
    {
      number: 53,
      meme: 'https://i.giphy.com/media/n43Kx7PeptN0k/giphy-downsized-large.gif',
    },
    {
      number: 65,
      meme: 'https://media1.giphy.com/media/YWf50NNii3r4k/giphy.gif?cid=ecf05e47lo65qtlvirrtjz7rowdoh0h4wt7zxv6zpndrukla&ep=v1_gifs_search&rid=giphy.gif&ct=g',
    },
    {
      number: 69,
      meme: '../assets/69.gif',
    },
    {
      number: 77,
      meme: 'https://media3.giphy.com/media/xUPGcjUQcWclgK94ti/giphy.gif?cid=ecf05e47506hf2zyf2vm6l6kk8r4qug0r6q5x7fc0tl9ywh2&ep=v1_gifs_search&rid=giphy.gif&ct=g',
    },
    {
      number: 79,
      meme: 'https://media.tenor.com/LM8RQJLOQNMAAAAC/789-why-was6afraid-of7.gif',
    },
    {
      number: 89,
      meme: 'https://media3.giphy.com/media/1Qdp4trljSkY8/giphy.gif?cid=ecf05e47yasjdfesft58rwyw3vx2xxfb1jj1ba7v0q8rucv8&ep=v1_gifs_search&rid=giphy.gif&ct=g',
    },
    {
      number: 90,
      meme: 'https://media.tenor.com/Lwn2wbmAXboAAAAd/skeleton.gif',
    },
    {
      number: -1,
      meme: 'https://media4.giphy.com/media/IwAZ6dvvvaTtdI8SD5/giphy.gif?cid=ecf05e47529nfucezv01a3porkqul3kc30kzah5rwggxriyc&ep=v1_gifs_search&rid=giphy.gif&ct=g',
    },
  ];
}
